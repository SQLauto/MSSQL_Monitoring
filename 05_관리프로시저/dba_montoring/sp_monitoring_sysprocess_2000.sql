--테이블 생성
/*********************************************
    파티션이 적용되지 않으므로 테이블 10개 생성
**********************************************/

CREATE TABLE MONITOR_SYSPROCESS_0
(
...
ALTER TABLE MONITOR_SYSPROCESS_0 ADD CONSTRAINT PK__MONITOR_SYSPROCESS_0__SEQ_NO  
    PRIMARY  KEY NONCLUSTERED (seq_no)
;

CREATE CLUSTERED INDEX CIDX__MONITOR_SYSPROCESS_0__REG_DT ON MONITOR_SYSPROCESS_0 (REG_DT)
;
    ...
    begin 
        insert into #monitoring_sysprocess
            (reg_dt,lastwaittype, dbname,spid,blocked,cpu,physical_io,status, waittype,waittime,waitresource,
             cmd, hostname,program_name,login_time,last_batch,sql_handle, stmt_start,stmt_end)
        select  
                getdate(),lastwaittype, db_name(dbid) as dbname, spid, blocked, 
                cpu,physical_io, status,
                waittype,waittime, waitresource,
                cmd, CONVERT(VARCHAR(25),hostname) as hostname, program_name,
...
SET @object_query = N'SELECT  @name=  name FROM ' + db_name(@dbid) 
        ...
        set objectid = @objectid, query_text = @query_text
        where spid = @spid
    ...
    BEGIN
      ...
      insert into MONITOR_SYSPROCES_9
            (reg_dt,lastwaittype, dbname,spid,blocked,cpu,physical_io,status, waittype,waittime,waitresource,
             objectid,cmd, query_text,hostname,program_name,login_time,last_batch,sql_handle, stmt_start,stmt_end)
      select reg_dt,lastwaittype, dbname,spid,blocked,cpu,physical_io,status, waittype,waittime,waitresource,
             objectid,cmd, query_text,hostname,program_name,login_time,last_batch,sql_handle, stmt_start,stmt_end
    ...
    begin                   
      if @day = 0 truncate table dbo.MONITOR_SYSPROCES_1             
      if @day = 1 truncate table dbo.MONITOR_SYSPROCES_2                  
      if @day = 2 truncate table dbo.MONITOR_SYSPROCES_3                  
      if @day = 3 truncate table dbo.MONITOR_SYSPROCES_4                  
      if @day = 4 truncate table dbo.MONITOR_SYSPROCES_5                  
      if @day = 5 truncate table dbo.MONITOR_SYSPROCES_6                  
      if @day = 6 truncate table dbo.MONITOR_SYSPROCES_7                  
      if @day = 7 truncate table dbo.MONITOR_SYSPROCES_8                  
      if @day = 8 truncate table dbo.MONITOR_SYSPROCES_9                  
      if @day = 9 truncate table dbo.MONITOR_SYSPROCES_0             
    end    
