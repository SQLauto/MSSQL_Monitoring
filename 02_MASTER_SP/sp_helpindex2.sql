...
-- http://www.SQLskills.com/blogs/Kimberly
	...
	select
		...
		'index_description' = convert(varchar(210), --bits 16 off, 1, 2, 16777216 on, located on group
				case when tab.index_id = 1 then 'clustered' else 'nonclustered' end
		...
		from #spindtab tab
			join (select index_id,  rows, data_compression_desc 
		   from sys.partitions with(nolock) 
		   where object_name(object_id) = @objname)par on tab.index_id = par.index_id
	order by index_name
