...
ALTER TABLE DBA_REINDEX_TARGET ADD CONSTRAINT PK__DBA_REINDEX_TARGET 
    primary key clustered ([id] ASC, [reg_date] ASC  ) ON [PRIMARY] 
GO
...
GO
ALTER TABLE DBA_REINDEX_SPACE ADD CONSTRAINT PK__DBA_REINDEX_SPACE__ID 
...
/*
 ...
 인덱스 rebuild 작업 처리   
    choi bo ra  
수정 내용 : 파티션된 인덱스가 있을 경우 처리 보강. 서은미, 2012-11-12 
*/
ALTER PROC up_dba_reindexes
...
DECLARE @index_nm nvarchar(130);    
DECLARE @table_nm nvarchar(100);
...
BEGIN            
	...
	,@index_nm = index_name
	, @table_nm = table_name
	FROM DBA.dbo.DBA_REINDEX_TARGET          
	WHERE              
	...
	select @table_name,@table_nm, @id,@index_name,@database_name, @schemaname,@objectname
	...
	BEGIN     
		...
		IF @partitionnum = 1
			UPDATE DBA.dbo.DBA_REINDEX_TARGET SET Execute_Status = 'P' WHERE id = @id  
		ELSE --파티션된 인덱스 일경우 전체파티션 인덱스 리빌드 처리, 서은미, 2012-11-12	
			UPDATE DBA.dbo.DBA_REINDEX_TARGET SET Execute_Status = 'P' WHERE database_name= @database_name and table_name = @table_nm and index_name = @index_nm
		...
		SET @command =  N'ALTER INDEX ' + @index_name + N' ON ' + @database_name + N'.' + @schemaname + N'.' + @objectname + N' REBUILD WITH (ONLINE = ON, MAXDOP=8, FILLFACTOR='+convert(varchar(10),@fill_factor) +' )';             
		...
		INSERT  DBA.DBO.DBA_REINDEX_SPACE           
		(            
		...
		IF @partitionnum = 1
			UPDATE DBA.dbo.DBA_REINDEX_TARGET SET Execute_Status = 'E', rebuild_date=GETDATE()  WHERE id = @id             
		ELSE --파티션된 인덱스 일경우 전체파티션 인덱스 리빌드 처리, 서은미, 2012-11-12
			UPDATE DBA.dbo.DBA_REINDEX_TARGET SET Execute_Status = 'E', rebuild_date=GETDATE() WHERE database_name= @database_name and table_name = @table_nm and index_name = @index_nm
...
from dbadmin.DBO.DB_REINDEX_TARGET_201008 a with(nolock)
...
--정보보기
select  a.execute_status,c.name as filegroupname, a.database_name , table_name,
...
from dbadmin.DBO.DBA_REINDEX_TARGET a with(nolock)
    inner join sys.indexes b on a.object_id = b.object_id and a.index_id=b.index_id
    inner join sys.filegroups c on b.data_space_id=c.data_space_id
where  a.avg_fragmentation_in_percent > 50
    AND database_name = 'itemdb1' 
   AND (''  = '' and table_name = table_name or table_name = '' )       
   AND partition_number = 1   
   and isnull(a.execute_status, 'P') != 'E'
order by avg_fragmentation_in_percent desc


select database_name, table_name,index_name, 
    before_reserved /128 as before_reserved,before_used/128 as before_used,before_rowcnt,
    after_reserved/128 asafter_reserved ,after_used/128 asafter_used ,after_rowcnt,
    (before_reserved -after_reserved) /128 as diff,
    duration,start_date,end_date
from dbadmin.dbo.DBA_REINDEX_SPACE with (nolock) 
where end_date >= '2011-01-11' order by end_date desc
