...
/*************************************************************************  
* 프로시저명  : dbo.up_dba_report_critical_size
* 작성정보    : 2010-
* 관련페이지  : 
* 내용        : 
* 수정정보    :
**************************************************************************/
CREATE PROCEDURE dbo.up_dba_report_critical_size 
AS
/* COMMON DECLARE */
SET NOCOUNT ON
...
select b.server_name,a.db_id, d.db_name,SUM(a.size) as [database_size], SUM(size)-SUM(a.usage) as [unallocated space]
...
group by b.server_name,a.db_id, d.db_name
ORDER BY b.server_name, d.db_name
