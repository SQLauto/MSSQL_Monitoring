...
. : DB  Index  
.     
  - exec UP_DBA_COLLECT_INDEX_DETAIL 'maindb2'
*****************************************************************************************************************   
...
BEGIN
	...
	SELECT DISTINCT
			...
			declare @count int
			SELECT @count=COUNT(*) FROM DBADMIN..DBA_REINDEX_TOTAL_LIST WITH(NOLOCK)
			if (@count = 0)
			BEGIN
				INSERT INTO DBADMIN..DBA_REINDEX_TOTAL_LIST
				(
				DB_NAME
				,SCHEMA_NAME
				,TABLE_NAME
			...
			BEGIN
				...
				FROM DBADMIN..DBA_REINDEX_TOTAL_LIST A WITH(NOLOCK) 
				...
				(
					select A.*, B.reg_dt as REG_DT 
						FROM #T A WITH(NOLOCK) 
						LEFT JOIN DBADMIN..DBA_REINDEX_TOTAL_LIST B WITH(NOLOCK) ON B.DB_NAME = '''+@DBName+''' and A.TABLENAME = B.TABLE_NAME and A.INDEX_ID = B.INDEX_ID 
